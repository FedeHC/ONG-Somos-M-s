...
const TestimonialForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await getTestimonials(id);
  ...
  const submitHandler = values => {
    if (location.includes('create')) createTestimony(values);
    else if (location.includes('edit')) updateTestimony(values, id);
    else if (location.includes('delete')) deleteTestimony(values, id);
    else return;
  };
              ...
              >
                ...
                <Center bg="#00214d" mt={0} mb={10} p={3} borderRadius={10}>
                  <Heading size="xl" color="white">
                    {id ? 'Editando' : 'Creando'} Testimonio
                  </Heading>
                </Center>
